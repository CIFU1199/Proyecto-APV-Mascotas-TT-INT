...
import { Box, Button, Typography, Paper, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Select, MenuItem, FormControl, InputLabel, TextField, Dialog, DialogTitle, DialogContent, DialogActions, Alert, Snackbar } from '@mui/material';
                      ...
                      <Typography
                        sx={{
                          color: cita.estado === 'Pendiente' ? 'orange' : 
                                cita.estado === 'Atendida' ? 'green' : 'red',
                          fontWeight: 'bold'
                        }}
                      >
                        {cita.estado}
            ...
            <DialogContent>
              <FormControl fullWidth margin="normal" error={!!errors.MACT_ID}>
                ...
                <Select
                  ...
                  id="MACT_ID"
                  name="MACT_ID"
                  value={formData.MACT_ID}
                  label="Mascota"
                  onChange={handleChange}
                >
                  <MenuItem value={0}>Selecciona una mascota</MenuItem>
                  {mascotas.map((mascota) => (
                    <MenuItem key={mascota.MACT_ID} value={mascota.MACT_ID}>
                      {mascota.MACT_NOMBRE}
                    </MenuItem>
                  ))}
                </Select>
                {errors.MACT_ID && <Typography color="error" variant="caption">{errors.MACT_ID}</Typography>}
              </FormControl>

              <Box sx={{ display: 'flex', gap: 2, mt: 2 }}>
                <FormControl fullWidth margin="normal" error={!!errors.CIT_FECHACITA}>
                  <DatePicker
                    label="Fecha de la cita"
                    value={formData.CIT_FECHACITA ? parse(formData.CIT_FECHACITA, 'yyyy-MM-dd', new Date()) : null}
                    onChange={handleDateChange}
                    minDate={new Date()}
                    format="dd/MM/yyyy"
                  />
                  {errors.CIT_FECHACITA && <Typography color="error" variant="caption">{errors.CIT_FECHACITA}</Typography>}
                </FormControl>

                <FormControl fullWidth margin="normal" error={!!errors.CIT_HORA}>
                  <TimePicker
                    label="Hora de la cita"
                    value={formData.CIT_HORA ? parse(formData.CIT_HORA, 'HH:mm:ss', new Date()) : null}
                    onChange={handleTimeChange}
                    minutesStep={15}
                  />
                  {errors.CIT_HORA && <Typography color="error" variant="caption">{errors.CIT_HORA}</Typography>}
                </FormControl>
              </Box>
